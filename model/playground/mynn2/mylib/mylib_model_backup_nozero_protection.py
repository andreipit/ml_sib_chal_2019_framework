...
class my_l_layers_ffn: 
    ...
    def __learning_rate_decay(self, learning_rate,  epoch_num, decay_type='no', decay_rate=0.95):
        if decay_type=='pop': r = learning_rate / (1 + decay_rate * epoch_num) #0.001 learning rate decay: alpha = alpha_0 / [1 + decay_rate * epoch_num]
        elif decay_type=='exp': r = decay_rate * epoch_num * learning_rate # alpha = 0.95^epoch_num * alpha_0
        elif decay_type=='discrete': r = decay_rate * learning_rate / np.sqrt(epoch_num) # not working
    ...
    def __forward(self, X, p, L, DO_keep_prob=1):
        ...
        def __linear_activation_forward(Z, activation):
            if activation == "sigmoid": A = 1/(1+np.exp(-Z)) # A = 1/(1+expit(-Z))  # vs RuntimeWarning: overflow encountered in exp
            elif activation == "sigmoid_fast":  A = 1/(1+np.exp(-Z)) # A = 1/(1+expit(-Z))  
            elif activation == "relu":  A = np.maximum(0,Z)
            elif activation == "tanh":  A = np.tanh(Z)
            elif activation == "softmax": Z_exp = np.exp(Z); Z_sum = np.sum(Z_exp, axis = 0, keepdims = True); A =  Z_exp / Z_sum # A = np.exp(Z)/np.sum(np.exp(Z), axis = 0, keepdims = True)
            return A
    ...
    def __backward(self, Y, c, L, m, L2_lambd, DO_keep_prob):
        ...
        def loss_backward(Y, AL, method, activation):
            if method == "log": 
                if activation == "sigmoid":   return -Y/AL + (1-Y)/(1-AL) #if method == "log_sig_fast": return AL - Y # log + sigmoid can be computed faster: dZL = AL - Y
                elif activation == "softmax": return -Y/AL + (1-Y)/(1-AL) # for softmax retun must be -Y/AL, but we use sigmoid version to get right derivative, cause I dont know dAdZ for softmax
